...
// waitDelayBuffer pads a scenario's own service.managed.stop_timeout when
// sizing how long RunScenario waits after sending SIGTERM before giving up
// and force-killing the child — the child's graceful shutdown (Stop, then
// teardown, then writing its report) takes a bit longer than
// stop_timeout alone bounds.
const waitDelayBuffer = 10 * time.Second

// defaultWaitDelay is used when the scenario has no service.managed at
// all (nothing to gracefully stop) — still generous enough for
...
// scenario's own service.managed.stop_timeout when it configures
// service.managed, so a generous per-scenario timeout doesn't get cut
// short by an unrelated, smaller default here.
//
...
// service.managed.stop_timeout — best-effort: if the config can't even be
// loaded, RunScenario's own subprocess will fail identically and report
// the real error, so a generic fallback here is fine. When
// skipServiceLifecycle is true, the child never stops a service itself
// (the suite process does, separately), so stop_timeout doesn't apply
// here.
func waitDelayForScenario(configPath string, skipServiceLifecycle bool) time.Duration {
	...
	if err != nil || cfg.Service.Managed == nil {
		return defaultWaitDelay
	}
	return cfg.Service.Managed.StopTimeout.Duration() + waitDelayBuffer
}
